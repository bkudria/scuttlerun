...
**Implementation note:** The `for await` loop over the SDK's async iterable does not respond to `AbortController` signals mid-iteration. On timeout, scuttlerun sets a `timedOut` flag, calls `abortController.abort()` (the controller is passed to the SDK via `sdkOptions.abortController`), and also calls `queryHandle.interrupt()` as a belt-and-suspenders signal to the SDK's child process. The `for await` loop checks `timedOut` at the top of each iteration and breaks; the abort signal additionally causes the SDK to end the iterator. Together these mean the loop exits promptly after the current in-flight message resolves — a hung SDK iteration is bounded by the SDK's own abort handling, not by scuttlerun.
